...
public class PluginsWithCrawlerTest {
    ...
    public static void setup() {
         ...
         */
        builder.crawlRules().addInvariant("Never contain Final state S8", new NotRegexCondition("Final state S2"));
        ...
        builder.addPlugin((OnNewStatePlugin) (context, state) -> {
            ...
            if (!state.getName().equals("index")) {
                assertTrue(
                        "currentState and indexState are never the same",
        ...
        builder.addPlugin((PostCrawlingPlugin) (session, status) -> plugins.add(PostCrawlingPlugin.class));
    ...
    private void afterFirstPluginsIsFollowedBy(
            Class<OnUrlLoadPlugin> suspect, Iterable<Class<? extends Plugin>> followedBy) {
        ...
        for (int index : indexes) {
            ...
            assertThat(
                    ...
                    followedBy,
                    IsCollectionContaining.hasItem(follower));
        }
    }
    ...
    public void verifyOnNewStateFollowers() {
        pluginsIsFollowedBy(
                OnNewStatePlugin.class,
                ImmutableSet.of(PreStateCrawlingPlugin.class, OnUrlLoadPlugin.class, PostCrawlingPlugin.class));
    }
    ...
    public void onRevisitStatesFollowers() {
        pluginsIsFollowedBy(
                OnRevisitStatePlugin.class,
                ImmutableSet.of(
                        OnFireEventFailedPlugin.class, OnInvariantViolationPlugin.class, OnNewStatePlugin.class));
    }
