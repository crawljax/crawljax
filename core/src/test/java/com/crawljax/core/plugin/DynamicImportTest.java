...
public class DynamicImportTest {
	...
	// as a developer, I want every folder to give me 0...n plugins, so my code
	// doesn't have to
	...
	public void ImportFromExistingFolderTest() {
		...
		List<Plugin> classPathPlugins = PluginImporter.getPluggedServices(
				Plugin.class, tempDir);
		assertEquals(0, classPathPlugins.size());
	}

	// as a crawljax user, I want my plugin to be picked up from a folder, so
	// that I can pickup
	...
	// as a crawljax user, I want to not have non plugin code included, so
	// crawljax doesn't have to
	...
	// as a developer, I want the system to abort if it doesn't find a folder,
	// so that the system
	// doesn't crash
	@Test
	...
	// as a user, I want the system to grab plugins from all subdirectories, so
	// that no special
	// folder structure is necessary
	@Test
	public void ImportFromSubdirectories() {
		File tempDir = SetupTestFolderStructure(new File(TEST_FOLDER_ADDRESS));
		File tempSubDir = SetupTestFolderStructure(new File(
				TEST_SUBFOLDER_ADDRESS));
		tempSubDir.mkdir();

		boolean FolderExists = tempDir.exists() | tempSubDir.exists();
		assertEquals(true, FolderExists);

		CreateJarFile.createJar(
				new File(tempSubDir.getPath() + "/testjar2.jar"),
				SamplePreCrawlingPlugin.class, true);
	...
	// as a user, if a plugin is present in both folder and jar form, only one
	// should be included
	@Test
