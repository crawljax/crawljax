...
public class StateFlowGraphVisualTest {
    ...
    @Before
    public void setup() {
        index = new DHashStateVertexImpl(StateVertex.INDEX_ID, "index", "<table><div>index</div></table>", "001");
        state2 = new DHashStateVertexImpl(2, "STATE_TWO", "<table><div>state2</div></table>", "002");
        state3 = new DHashStateVertexImpl(3, "STATE_THREE", "<table><div>state3</div></table>", "003");
        state4 = new DHashStateVertexImpl(4, "STATE_FOUR", "<table><div>state4</div></table>", "004");
        state5 = new DHashStateVertexImpl(5, "STATE_FIVE", "<table><div>state5</div></table>", "005");
    ...
    public void testDuplicationAdding() {
        ...
        StateVertex stateReturned = graph.putIfAbsent(
                new DHashStateVertexImpl(12, "STATE_12", "<table><div><span>state22</span></div></table>", "002"));
        ...
        assertTrue(state2.equals(new DHashStateVertexImpl(2, "STATE_2", "<table><div>state2</div></table>", "002")));
        ...
        StateVertex hold = new DHashStateVertexImpl(StateVertex.INDEX_ID, index.getName(), index.getDom(), "001");
    ...
    public void testCloneStates() {
        StateVertex state3clone2 =
                new DHashStateVertexImpl(3, "STATE_THREE", "<table><div>state2</div></table>", "002");

        assertTrue(graph.putIfAbsent(state2) == null);
        assertTrue(graph.putIfAbsent(state4) == null);
        assertTrue(graph.addEdge(index, state2, newXpathEventable("/body/div[4]")));

        assertTrue(graph.addEdge(
                state4, state3clone2, new Eventable(new Identification(How.xpath, "/home/a"), EventType.click)));
