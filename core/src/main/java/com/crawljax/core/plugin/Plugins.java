...
public class Plugins {
    ...
    private void addPlugins(
            List<? extends Plugin> plugins, ImmutableListMultimap.Builder<Class<? extends Plugin>, Plugin> builder) {
        ...
        for (Plugin plugin : plugins) {
            for (Class<?> clazz : plugin.getClass().getInterfaces()) {
                if (KNOWN_PLUGINS.contains(clazz)) {
                    ...
                    LOGGER.info("Loaded {} as a {}", plugin, clazz.getSimpleName());
        ...
        if (!unusedPlugins.isEmpty()) {
            LOGGER.warn(
                    "These plugins were added but are ignored because they are unknown to Crawljax, {}", unusedPlugins);
        }
    }
    ...
    private void incrementFailCounterFor(Plugin plugin) {
        registry.counter(MetricsModule.PLUGINS_PREFIX + plugin.getClass().getSimpleName() + ".fail_count")
                .inc();
    }

    /**
     * load and run the OnUrlLoadPlugins. The OnUrlLoadPlugins are run just after the Browser has gone
     * to the initial url. Not only the first time but also every time the Core navigates back.
     * Warning the instance of the browser offered is not a clone but the current and after wards used
     * browser instance, changes and operations may cause 'strange' behaviour.
     * <p>
    ...
    /**
     * load and run the OnNewStatePlugins. OnNewStatePlugins are plugins that are ran when a new state
     * was found. This also happens for the Index State. Warning the session is not a clone, changing
     * the session can cause strange behaviour of Crawljax.
     * <p>
     * This method can be called from multiple threads with different {@link CrawlerContext}
     * </p>
     *
    ...
    /**
     * Run the OnInvariantViolation plugins when an Invariant is violated. Invariant are checked when
     * the state machine is updated that is when the dom is changed after a click on a clickable. When
     * a invariant fails this kind of plugins are executed. Warning the session is not a clone,
     ...
     * finished Warning: changing the session can change the behavior of other post crawl plugins. It
     * is not a clone!
     *
     ...
     */
    public void runPreStateCrawlingPlugins(
    ...
    /**
     * Load and run the OnFireEventFailedPlugins, this call has been made from the fireEvent when the
     * event is not fireable. the Path is the Path leading TO this eventable (not included).
     *
    ...
    public void runOnBrowserClosingPlugins(CrawlerContext context) {
        ...
        for (Plugin plugin : plugins.get(OnBrowserClosePlugin.class)) {
            if (plugin instanceof OnBrowserClosePlugin) {
                LOGGER.debug("Calling plugin {}", plugin);
                try {
                    ((OnBrowserClosePlugin) plugin).onBrowserClose(context);
                } catch (RuntimeException e) {
                    reportFailingPlugin(plugin, e);
                }
            }
