...
public class CrawlTaskConsumer implements Callable<Void> {
  ...
  @Inject
  CrawlTaskConsumer(UnfiredFragmentCandidates candidates,
  ...
  private void pollAndHandleCrawlTasks() throws InterruptedException {
    try {
      ...
      if (crawler.getCrawlRules().isDelayNearDuplicateCrawling()) {
        crawlTask = candidates.awaitNewTask(crawler.getContext().getCurrentState(),
            crawler.getOnUrlSet(), crawler.getContext().getFragmentManager());
        if (crawlTask == null) {
