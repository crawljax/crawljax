...
public class StateMachine {
    ...
    public StateVertex newStateFor(EmbeddedBrowser browser) {
        return stateFlowGraph.newStateFor(
                browser.getCurrentUrl(), browser.getStrippedDom(), stateComparator.getStrippedDom(browser), browser);
    }

    /**
     * Change the currentState to the nextState if possible. The next state should already be present
     * in the graph.
     *
    ...
    public boolean changeState(StateVertex nextState) {
        ...
        LOGGER.debug("Trying to change to state: '{}' from: '{}'", nextState.getName(), currentState.getName());

        if (stateFlowGraph.canGoTo(currentState, nextState)) {

            LOGGER.debug("Changed to state: '{}' from: '{}'", nextState.getName(), currentState.getName());

            setCurrentState(nextState);

            return true;
        } else {
            LOGGER.info("Cannot go to state: '{}' from: '{}'", nextState.getName(), currentState.getName());
            return false;
        }
    }
    ...
    private StateVertex addStateToCurrentState(StateVertex newState, Eventable eventable) {
        LOGGER.debug("addStateToCurrentState currentState: {} newState {}", currentState.getName(), newState.getName());
        ...
        if (cloneState != null) {
            LOGGER.info("CLONE State detected: {} and {} are the same.", newState.getName(), cloneState.getName());
     ...
     */
    public boolean switchToStateAndCheckIfClone(final Eventable event, StateVertex newState, CrawlerContext context) {
    ...
    public void runOnInvariantViolationPlugins(CrawlerContext context) {
        for (Invariant failedInvariant : invariantChecker.getFailedConditions(context.getBrowser())) {
