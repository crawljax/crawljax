...
public class StatePair {
    ...
    public StatePair(
            StateVertex state1,
            StateVertex state2,
            List<Node> state1Nodes,
            List<Node> state2Nodes,
            StateComparision stateComparision) {
        this.state1 = state1;
        this.state2 = state2;
    ...
    public boolean equals(Object obj) {
        if (obj instanceof StatePair) {
            ...
            return ((this.state1.getId() == other.state1.getId())
                            && (this.state2.getId() == other.getState2().getId()))
                    || ((this.state2.getId() == other.state1.getId())
                            && (this.state1.getId() == other.getState2().getId()));
        }
        return false;
    }
