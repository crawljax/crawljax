 ...
 */
@Singleton
public class Sfg1 implements Serializable,StateFlowGraph {
	 ...
	 */
	@Inject
	public Sfg1(ExitNotifier exitNotifier) {
		...
		Sfg1.structuralIndexer = indexNode;
		...
		synchronized(sfgDb){
			...
			try {
				...
				if (alreadyEsixts != null) {
					...
					LOG.debug("putIfAbsent: Graph already contained vertex {}", state);
					...
					// the serialized stateVertex and the Stripped DOM are used for crawling 
					// purposes too!
					...
					if (name != null){
						toBeAddedNode.setProperty(STATE_NAME_IN_NODES, state.getName());
					}else{
...
//		combinedEdgeKey[1] = UTF8.decode(serializedEventable);
		...
		synchronized(sfgDb){
			...
			try {
				...
				alreadyExists = edgePutIfAbsent(toBeAddedEdge, SOURCE_CLICKABLE_TARGET_IN_EDGES_FOR_UNIQUE_INDEXING,
						combinedEdgeKey);
	...
	public StateVertex getTargetState(Eventable clickable) {
		...
		Relationship edge = (Relationship) edgesIndex.get(SERIALIZED_CLICKABLE_IN_EDGES,
		...
		try{
			...
			for (Relationship relationship: structuralIndexer.getRelationships(Direction.OUTGOING, RelTypes.INDEXES)){
				for (Relationship edge:relationship.getEndNode().getRelationships(Direction.OUTGOING, RelTypes.TRANSITIONS_TO)){
					...
					byte[] serializededge = (byte[]) edge.getProperty(SERIALIZED_CLICKABLE_IN_EDGES);
					Eventable eventable = deserializeEventable(serializededge, 1);
					
					Node startNode = edge.getStartNode();
					byte[] serializedStartNode = (byte[])startNode.getProperty(SERIALIZED_STATE_VERTEX_IN_NODES);
					StateVertex startState= deserializeStateVertex(serializedStartNode);
					eventable.setSourceStateVertex(startState);

					Node endNode = edge.getEndNode();
					byte [] serializedEndNode = (byte[])endNode.getProperty(SERIALIZED_STATE_VERTEX_IN_NODES);
