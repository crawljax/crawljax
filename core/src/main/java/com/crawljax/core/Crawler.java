...
public class Crawler {
  ...
  private Eventable getResetEquivalent() {
    ...
    try {
      if (stateMachine.getStateFlowGraph().canGoTo(stateMachine.getCurrentState(),
          stateMachine.getStateFlowGraph().getInitialState())) {
        Eventable returnEvent = stateMachine.getStateFlowGraph()
            .getShortestPath(stateMachine.getCurrentState(),
                stateMachine.getStateFlowGraph().getInitialState()).get(0);
        LOG.error("Found direct edge between current {} and index :  {} ",
            stateMachine.getCurrentState().getName(), returnEvent.getId());
        return returnEvent;
      }
    } catch (Exception ex) {
    ...
    ImmutableSet<Eventable> toReachIndex = stateMachine.getStateFlowGraph()
    ...
    for (Eventable toReach : toReachIndex) {
      try {
        CandidateElement elementToReach = toReach.getSourceStateVertex()
            .getCandidateElement(toReach.getElement().getNode()).get(0);
        CandidateElement equivalentCandidate = stateMachine.getCurrentState()
            .getCandidateElement(toReach);
        if (equivalentCandidate != null && fragmentManager.areRelated(
            elementToReach.getClosestFragment(), equivalentCandidate.getClosestFragment())) {
          LOG.info("Found equivalent edge from {}. Using it on {}",
              toReach.getSourceStateVertex().getName(), stateMachine.getCurrentState().getName());
  ...
  public void reset(int nextTarget) {
    ...
    if (UseEquivalentReset) {
      if (stateMachine != null && stateMachine.getCurrentState()
        ...
        if (equivalentEvent != null && fireEventWithInputs(equivalentEvent,
            equivalentEvent.getRelatedFormInputs())) {
          ...
          if (stateMachine.getCurrentState()
            ...
            LOG.info(
                "Action didn't result in index state. Disabling equivalent reset from next time");
          }
        }
      }
    }
    ...
    if (path != null) {
      try {
        ...
        if (!crawlTask.equals(stateMachine.getCurrentState())) {
          ...
          LOG.info("Tried reaching {} but Reached a near duplicate {}", crawlTask.getName(),
              stateMachine.getCurrentState().getName());
  ...
  private ArrayList<Eventable> followNew(CrawlPath path, StateVertex targetState)
    ...
    if (currState == null) {
      LOG.info("Backtracking Error!! starting from {}",
    ...
    for (Eventable clickable : path) {
      ...
      if (!clickable.getSourceStateVertex().equals(currState)) {
        ...
        if (avoidDifferentBacktracking &&
          ...
          LOG.info("Avoiding different backtracking -> {} {}", clickable.getSourceStateVertex(),
              currState);
          throw new StateUnreachableException(clickable.getSourceStateVertex(),
              String.format("The path didn't result in the desired %1$ but in %2$",
        ...
        if (newCandidate != null) {
          ...
          if (!fragmentManager.areRelated(newCandidate.getClosestFragment(),
              oldCandidate.getClosestFragment())) {
            ...
            if (avoidUnrelatedBacktracking) {
              ...
              throw new StateUnreachableException(targetState,
                  "Avoiding backtracking through unrelated fragments");
            } else {
          ...
          if (!newCandidate.isDirectAccess()) {
            ...
            LOG.info("Backtracking event {} explored already!! for equivalent candidate {}",
                clone.getId(), newCandidate.getIdentification().getValue());
          }
        } else {
          LOG.error("Could not find equivalent element in backtracking {} for candidate  {}",
              clone.getId(), clone.getIdentification().getValue());
        }
...
//							handleInputElements(clickable);
      ...
      Eventable added = graphProvider.get()
          .getShortestPath(clone.getSourceStateVertex(), clone.getTargetStateVertex()).get(0);
      LOG.info("Added to Path {}", added.getId());
      followedPath.add(added);

      LOG.info("Checking if event {} reached intended target {}", added.getId(),
      ...
      if (!clickable.getTargetStateVertex().equals(stateMachine.getCurrentState())) {
        ...
        LOG.info("Backtracking state has changed from {} to {} ",
            clickable.getTargetStateVertex().getName(), stateMachine.getCurrentState());
        ...
        if (clickable.getSourceStateVertex().equals(currState)) {
          ...
          LOG.info(" Removing expired Edge {} to {}", clickable.getId(),
              clickable.getTargetStateVertex().getName());
        ...
        if (stateMachine.getCurrentState() instanceof HybridStateVertexImpl) {
          ...
          LOG.info("changed backtracking state {} and  {} are {}", newState.getName(),
              expectedState.getName(), comp);
          StateComparision prevComp = fragmentManager.cacheStateComparision(
              clickable.getSourceStateVertex(), currState, true);
          if (avoidDifferentBacktracking && comp == StateComparision.DIFFERENT
            ...
            LOG.warn("Removing {} and all its connected edges from the graph",
                expectedState.getName());
            graphProvider.get().removeState(expectedState);
            throw new StateUnreachableException(expectedState,
                String.format("The path didn't result in the desired %1$ but in %2$", expectedState,
                    newState));
          }
        }

        LOG.info("Finding shortest path from {} to {}", stateMachine.getCurrentState().getName(),
            targetState.getName());
        CrawlPath newPath = null;
        try {
          newPath = CrawlPath.copyOf(shortestPathTo(targetState, stateMachine.getCurrentState()),
              targetState.getId());
        } catch (Exception ex) {
        ...
        if (newPath != null) {
          ...
          if (newPathLength < oldPathLength) {
            LOG.info("Found shorter path from chngwd bt state {} to {}",
                stateMachine.getCurrentState().getName(), targetState.getName());
        ...
        LOG.debug("Reached the intended backtracking state {} using {}",
            stateMachine.getCurrentState().getName(), clone.getId());
      }
      currState = stateMachine.getCurrentState();
    ...
    if (!currState.equals(targetState)) {
      boolean assignDynamic = true;
      StateComparision comp = fragmentManager.cacheStateComparision(currState, targetState,
          assignDynamic);
...
//					avoidPath(crawlpath);
  ...
  private List<FormInput> getInputElements(Eventable eventable) {
    ...
    LOG.info("Changing related inputs for {} with {}  to {} ", eventable.getId(),
        formInputsExisting.size(), formInputs.size());
    return formInputs;
  }
  ...
  private List<FormInput> orderFormInputs(List<FormInput> formInputs) {
    if (stateMachine == null
        || !(stateMachine.getCurrentState() instanceof HybridStateVertexImpl)) {
      return formInputs;
    }
    ...
    for (FormInput input : formInputs) {
      try {
        Node inputNode = XPathHelper.getBelongingNode(input,
            stateMachine.getCurrentState().getDocument());
        if (inputNode != null) {
...
//				handleInputElements(event);
  ...
  private void crawlThroughActionsNew() {
    ...
    boolean interrupted = Thread.interrupted();
    CandidateCrawlAction action =
        candidateActionCache.pollActionOrNull(stateMachine, context.getFragmentManager(),
            afterBacktrack);
    ...
    while (action != null && !interrupted) {
      ...
      if (element.allConditionsSatisfied(browser)) {
        ...
        if (fired) {
          try {
            fragmentManager.recordAccess(action.getCandidateElement(),
                stateMachine.getCurrentState());
          } catch (Exception ex) {
          ...
          LOG.info(
              "Recording direct access to the action to avoid picking in the same state again");
      ...
      action = candidateActionCache.pollActionOrNull(stateMachine, context.getFragmentManager(),
          afterBacktrack);
//			}
      interrupted = Thread.interrupted();
      if (!interrupted && crawlerLeftDomain()) {
  ...
  private void interruptThread() {
    ...
    if (crawlpath != null) {
      ...
      FragmentationPlugin.writeCrawlPath(context.getSession(), crawlpath,
      ...
      if (event.getId() == -1) {
        ...
        for (Eventable edge : stateMachine.getStateFlowGraph().getAllEdges()) {
          if (edge.equals(event)) {
            crawlpath.add(edge);
            LOG.info("CrawlPath fixed !! The eventable fired was a clone of existing edge{}",
                edge.getId());
            fixed = true;
            break;
        ...
        if (!fixed) {
          LOG.info("Crawlpath could not be fixed with graph. Using the removed eventable {}",
              removed.getId());
