...
public class UnfiredCandidateActions {
     ...
     * @throws InterruptedException when taking from the queue is interrupted.
     */
    public StateVertex awaitNewTaskPriority(
            CrawlPriorityMode crawlPriorityMode, boolean crawlNearDuplicates, boolean delayNearDuplicateCrawling)
        ...
        if (delayNearDuplicateCrawling) {
            if (retVertex.hasNearDuplicate()) {
                ...
                if (nextUnique != null) {
                    ...
                    LOG.info("All remaining states are near duplicates. Returning : {}", retVertex.getName());
                }
            }
        }
        return retVertex;
