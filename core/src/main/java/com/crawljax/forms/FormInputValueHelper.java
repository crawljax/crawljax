...
public final class FormInputValueHelper {
     ...
     */
    private FormInputValueHelper(InputSpecification inputSpecification, FormFillMode formFillMode) {
    ...
    public static synchronized FormInputValueHelper getInstance(
        ...
        if (instance == null) {
            instance = new FormInputValueHelper(inputSpecification, formFillMode);
        }
        return instance;
    }
        ...
        if (in.exists()) {
            ...
            try {
                deserialized = gson.fromJson(
                        FileUtils.readFileToString(in, Charset.defaultCharset()),
                        new TypeToken<List<FormInput>>() {}.getType());
    ...
    public List<CandidateElement> getCandidateElementsForInputs(
            EmbeddedBrowser browser, Element sourceElement, EventableCondition eventableCondition) {
        ...
        try {
            final Document dom = DomUtils.asDocument(browser.getStrippedDomWithoutIframeContent());
            ...
            for (int curValueIndex = 0; curValueIndex < maxValues; curValueIndex++) {
                ...
                CandidateElement candidateElement = new CandidateElement(
                        sourceElement, XPathHelper.getXPathExpression(sourceElement), formInputsForCurrentIndex);
    ...
    public Node getBelongingNode(FormInput input, Document dom) throws XPathExpressionException {
        ...
        switch (input.getIdentification().getHow()) {
            ...
            default:
                LOGGER.info("Identification {} not supported yet for form inputs.", input.getIdentification());
                break;
        }
    ...
    private Identification getIdentification(Node element) {
        ...
        if (attributes.getNamedItem("id") != null && formFillMode != FormFillMode.XPATH_TRAINING) {
            return new Identification(
                    Identification.How.id, attributes.getNamedItem("id").getNodeValue());
        } else if (attributes.getNamedItem("name") != null && formFillMode != FormFillMode.XPATH_TRAINING) {
            return new Identification(
                    Identification.How.name, attributes.getNamedItem("name").getNodeValue());
        }
     ...
     * @param input          The form input.
     */
    public void addTrainingInput(Identification identification, FormInput input) {
    ...
    private FormInput getFormInput(EmbeddedBrowser browser, Node element, int indexValue) {
        ...
        if (matchedInput != null) {
            ...
            try {
                FormInput input = new FormInput(getFormInputType(element), getIdentification(element));
    ...
    private FormInput formInputMatchingNode(Node element) {
        ...
        if (attributes.getNamedItem("name") != null && formFillMode != FormFillMode.XPATH_TRAINING) {
            id = new Identification(
                    Identification.How.name, attributes.getNamedItem("name").getNodeValue());
            FormInput input = this.formInputs.get(id);
            if (input != null) {
