...
public class CrawlSessionProvider implements Provider<CrawlSession> {
    ...
    @Inject
    public CrawlSessionProvider(
            InMemoryStateFlowGraph stateFlowGraph, CrawljaxConfiguration config, MetricRegistry registry) {
    ...
    public void setup(StateVertex indexState, CrawlTaskConsumer firstConsumer) {
        if (!isSet.getAndSet(true)) {
            ...
            session = new CrawlSession(
                    config, firstConsumer.getContext().getFragmentManager(), stateFlowGraph, indexState, registry);
        } else {
