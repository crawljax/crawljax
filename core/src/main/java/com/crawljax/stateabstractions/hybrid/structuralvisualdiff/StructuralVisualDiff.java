...
public class StructuralVisualDiff {
  ...
  private StructuralVisualDiff(StateVertexForElementsWithVisualInfo oldVertex,
  ...
  private void detectNodeModifications() {
    ...
    for (String addedNodeXPath : Lists.newArrayList(addedNodes)) {
      DOMElementWithVisualInfo newElementWithVisualInfo = getNewElementWithVisualInfo(
          addedNodeXPath);
      if (null != newElementWithVisualInfo && newElementWithVisualInfo.getVisualHash() != null) {
        ...
        for (String removedNodeXPath : Lists.newArrayList(removedNodes)) {
          ...
          DOMElementWithVisualInfo oldElementWithVisualInfo = getOldElementWithVisualInfo(
              removedNodeXPath);
          if (null != oldElementWithVisualInfo
              && oldElementWithVisualInfo.getVisualHash() != null) {
    ...
    for (String movedNodeXPath : Lists.newArrayList(movedNodes)) {
      if (movedNodes.contains(
          movedNodeXPath)) { // Necessary since we might have removed a node already!
        try {
