...
public class StructuralVisualStateVertexFactory extends StateVertexFactory {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(StructuralVisualStateVertexFactory.class.getName());
    ...
    public StateVertexForElementsWithVisualInfo newStateVertex(
            int id,
            String url,
            String name,
            String dom,
    ...
    private List<DOMElementWithVisualInfo> getElementsVisualInfo(EmbeddedBrowser browser, String strippedDom) {
        ...
        try {
            ...
            populateDOMNodesWithVisualInfo(
                    browser,
                    domElements,
    ...
    private void populateDOMNodesWithVisualInfo(
            EmbeddedBrowser browser, List<DOMElementWithVisualInfo> domElements, Node node) {
        ...
        if (shouldDoVisualComaprison(node)) {
            ...
            if (node instanceof TextImpl) {
                ...
                if (boundingBox.getWidth() == 0 || boundingBox.getHeight() == 0) {
                    LOGGER.warn(
                            "Node {} is not visible. Take extra care about it. Dimensions are {}",
                            xpath,
                            DOMElementWithVisualInfo.getBoundingBoxDimensionsString(boundingBox));
                } else {
    ...
    private BufferedImage getElementScreenshot(Rectangle boundingBox) {
        BufferedImage screenshotImage = screenshot.getSubimage(
    ...
    private Rectangle getNodeRectangle(EmbeddedBrowser browser, String xpath) {
        ...
        javascript =
                "return document.evaluate(\"%s\", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue.getBoundingClientRect();";
