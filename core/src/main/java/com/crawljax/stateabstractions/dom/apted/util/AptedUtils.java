...
public class AptedUtils {
    ...
    private static AptedNode<StringNodeData> createTree(TreeWalker walker) {
        ...
        AptedNode<StringNodeData> node =
                new AptedNode<>(new StringNodeData(getNodeStringRepresentation(parent, false)));
    ...
    public static AptedNode<StringNodeData> getAptedTree(Node node, boolean visualData) {
        ...
        AptedNode<StringNodeData> root =
                new AptedNode<>(new StringNodeData(getNodeStringRepresentation(node, visualData)));
        ...
        if (node.getNodeName().equalsIgnoreCase("select")) {
            ...
            for (Node child : VipsUtils.getChildren(node)) {
                if (child.getNodeName().equalsIgnoreCase("option")) {
                    AptedNode<StringNodeData> option =
                            new AptedNode<>(new StringNodeData(getNodeStringRepresentation(child, visualData)));
