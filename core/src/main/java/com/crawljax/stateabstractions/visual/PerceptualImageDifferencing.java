...
public class PerceptualImageDifferencing {
   ...
   */
  public PerceptualImageDifferencing(double fieldOfView, int thresholdPixels, boolean failFast,
      double gamma, double luminance,
   ...
   * <code>dst</code>.
   * <p>
   * Adapted from <a href="http://www.jhlabs.com/ip/GaussianFilter.java">Jerry Huxtable's Gaussian
   * Filter</a>
   */
  ...
  /**
   * Computes the contrast sensitivity function (Barten SPIE 1989) given the cycles per degree (cpd)
   * and luminance (lum).
   */
   ...
   *
   * <p>
   * For an implementation with adjustable accuracy, see article by Harrison Ainsworth: <a href=
   * "http://www.hxa.name/articles/content/fast-pow-adjustable_hxa7241_2007.html" > Fast pow() With
   * Adjustable Accuracy</a>.
   *
   * @param a the base
  ...
  public void dump() {
    ...
    System.out.println(
        String.format("The display's Luminance is %s candelas per meter squared", luminance));
  }
  ...
  public static class Builder {
    ...
    public PerceptualImageDifferencing build() {
      return new PerceptualImageDifferencing(fieldOfView, thresholdPixels, failFast, gamma,
          luminance, luminanceOnly, colorFactor);
    }
  }
  ...
  private class PyramidTask implements Runnable {
    ...
    protected PyramidTask(int[] rgb, float[] a, float[] b, float[][] levels, int width,
        int height) {
      this.rgb = rgb;
      this.a = a;
      this.b = b;
  ...
  /**
   * ForkJoin idioms adapted from {@link java.util.concurrent.RecursiveAction sumOfSquares} sample.
   */
