...
public class FragmentationPlugin implements OnNewStatePlugin, OnRevisitStatePlugin,
    PostCrawlingPlugin {
...
//					manager.addFragment(fragment);
  ...
  public void postCrawling(CrawlSession session, ExitStatus exitReason) {
    ...
    CopyOnWriteArrayList<StateVertex> allStates = new CopyOnWriteArrayList<>(
        session.getStateFlowGraph().getAllStates());
      ...
      for (StateVertex state1 : allStates) {
        for (StateVertex state2 : allStates) {
          ...
          try {
            ...
            StateComparision comp = session.getFragmentManager()
                .cacheStateComparision(state2, state1, assignDynamic);
...
//			System.out.println(fragOutput);
    ...
    for (int i = 0; i < session.getCrawlPaths().size(); i++) {
      ...
      if (path instanceof CrawlPath) {
        ...
        CrawlPathInfo pathInfo = new CrawlPathInfo(i + 1, crawlPath.getBacktrackTarget(),
            crawlPath.isBacktrackSuccess(), crawlPath.isReachedNearDup());
