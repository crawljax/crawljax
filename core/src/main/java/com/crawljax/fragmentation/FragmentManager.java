...
public class FragmentManager {
    ...
    for (Fragment fragment : fragments) {
      if (fragment.getChildren().isEmpty()) {
        if (usefulFragment(fragment)) {
          ...
          if (addedParent) {
            LOG.debug("Added parent{} to the leaf node because child {} is not useful",
                parent.getId(), fragment.getId());
          }
        }
      }
    }
  ...
  public void cacheStateComparision(StatePair statePair, boolean assignDynamic) {
    if (!stateComparisionCache.containsKey(statePair)) {
      ...
      if (assignDynamic && (comp.equals(StateComparision.NEARDUPLICATE1) || comp.equals(
          StateComparision.NEARDUPLICATE2))) {
        List<Fragment> dynamic = ((HybridStateVertexImpl) statePair.getState1()).assignDynamicFragments(
            statePair.getState2());
...
//		coveredCandidates = new ArrayList<CandidateElement>();
    ...
    if (state.hasNearDuplicate() && state.getRootFragment() != null) {
      ...
      for (StateVertex nearDuplicate : nearDuplicateStates) {
        ...
        if (equivalentCandidates != null && !equivalentCandidates.isEmpty()) {
          ...
          LOG.warn("Could not record equivalent access for {} in {}", element.getIdentification(),
              equivalentFragment.getReferenceState().getName());
        }
      }
    }
  ...
  private void recordEquivalentAccess(CandidateElement element,
      ArrayList<CandidateElement> coveredCandidates,
  ...
  private void recordDuplicateAccess(CandidateElement element,
      ArrayList<CandidateElement> coveredCandidates,
      Fragment closestFragment) {
    ...
    for (Fragment duplicateFragment : duplicateFragments) {
      ...
      List<CandidateElement> duplicateCandidates = duplicateFragment.recordDuplicateCandidateAccess(
          element,
          closestFragment, coveredCandidates);
  ...
  public StateVertex getClosestUnexploredState(StateVertex currentState, List<StateVertex> onURLSet,
    ...
    for (StateVertex state : sfg.get().getAllStates()) {
      ...
      if (unexploredStateFound && !unexploredNearDuplicateFound && !hasExploredNearDuplicate(
          state)) {
      ...
      try {
        ...
        if (applyNonSelAdvantage) {
          ...
          LOG.info("{} : candidate {}, hop{}, nonSel {}, influence {}", state.getName(),
              candidateInfluence, hopInfluence, nonSelectionAdvantage, influence);
        } else {
          LOG.info("{} : candidate {}, hop{}, influence {}", state.getName(), candidateInfluence,
              hopInfluence, influence);
        }
  ...
  public double calculateCandidateInfluence(CandidateElement candidate) {
    ...
    candidateInfluence = candidateInfluence - (0.5 * candidate.getDuplicateAccess()
        + 0.25 * candidate.getEquivalentAccess());
...
//			LOG.error("No Close fragment?" + element + " in : " + state.getName());
  ...
  public StateComparision cacheStateComparision(StateVertex newState, StateVertex expectedState,
    ...
    if (getCachedComparision(newState, expectedState) == null) {
      ...
      if (newState.equals(expectedState)) {
        ...
        return comp;
      } else if (getRelatedFragments(newState.getRootFragment()).contains(
          expectedState.getRootFragment())) {
       ...
       */
      List<List<Node>> differentNodes = ((HybridStateVertexImpl) newState).getDifference(
          expectedState);
      ...
      for (Fragment affectedFragment : affectedNewFragments) {
        ...
        for (Fragment relatedFragment : relatedFragments) {
          if (relatedFragment.getReferenceState().getId() == expectedState.getId()) {
            LOG.debug("Mapped affected Fragment {} {} to {} {}", newState.getName(),
      ...
      for (Fragment affectedFragment : affectedOldFragments) {
        ...
        for (Fragment relatedFragment : relatedFragments) {
          if (relatedFragment.getReferenceState().getId() == newState.getId()) {
            LOG.debug("Mapped affected Fragment {} {} to {} {}", expectedState.getName(),
                affectedFragment.getId(),
  ...
  public void cacheStateComparisions(StateVertex newState) {
    ...
    CopyOnWriteArrayList<StateVertex> allStates = new CopyOnWriteArrayList<>(
        sfg.get().getAllStates());
