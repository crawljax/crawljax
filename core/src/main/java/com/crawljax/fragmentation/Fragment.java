...
public class Fragment {
  ...
  public Mat getChist() {
    if (this.chist == null) {
      ...
      BufferedImage thisImage = ((HybridStateVertexImpl) referenceState).getImage()
  ...
  public FragmentComparision compare(Fragment other) {
    ...
    try {
      ...
      LOG.error("Error comparing Fragment images {} {} : {} {}", this.id,
          this.referenceState.getName(), other.getId(), other.getReferenceState().getName());
    }
  ...
  public void setFragmentParentNode(Node fragmentParentNode) {
    ...
    if (!set) {
      LOG.error("node{} already a parent for fragment {}", fragmentParentNode,
  ...
  public List<CandidateElement> recordDuplicateCandidateAccess(CandidateElement element,
      Fragment duplicateFragment, List<CandidateElement> coveredCandidates) {
  ...
  public List<CandidateElement> recordEquivalentCandidateAccess(CandidateElement element,
      Fragment duplicateFragment, List<CandidateElement> coveredCandidates) {
    List<CandidateElement> returnList = new ArrayList<>();
    for (CandidateElement coveredCandidate : coveredCandidates) {
      if (this.candidates.contains(coveredCandidate)) {
  ...
  private List<CandidateElement> getEquivalentCandidate(CandidateElement element,
    ...
    try {
      if (duplicateFragment.fragmentParentNode == null) {
        ...
        String relativeXpath = XPathHelper.getXPathFromSpecificParent(element.getElement(),
            tempDuplicateParent);
      ...
      String relativeXpath = XPathHelper.getXPathFromSpecificParent(element.getElement(),
          duplicateFragment.getFragmentParentNode());
      Node equivalentNode = XPathHelper.getNodeFromSpecificParent(fragmentParentNode,
          relativeXpath);
  ...
  public void setEquivalentCoverage(Node equivalentNode, Fragment equivalentFrag,
      AccessType indirect, Coverage coverage) {
...
//		VipsUtils.setAccessType(node, indirect);
  ...
  public Node getEquivalentNode(Node element, Fragment duplicateFragment) {
    try {
      ...
      String relativeXpath = XPathHelper.getXPathFromSpecificParent(element,
          duplicateFragment.getFragmentParentNode());
      Node equivalentNode = XPathHelper.getNodeFromSpecificParent(fragmentParentNode,
          relativeXpath);
      return equivalentNode;
  ...
  public void transferCoverage(Fragment oldFragment) {
    try {
      for (Node node : oldFragment.getAllNodes()) {
        setEquivalentCoverage(node, oldFragment, AccessType.equivalent,
            VipsUtils.getCoverage(node, AccessType.direct));
        setEquivalentCoverage(node, oldFragment, AccessType.equivalent,
            VipsUtils.getCoverage(node, AccessType.equivalent));
      }
    } catch (Exception ex) {
      LOG.error("Error transferring coverage for {}-{} from {}-{} ", this.referenceState.getId(),
          this.getId(), oldFragment.getReferenceState().getId(), oldFragment.getId());
