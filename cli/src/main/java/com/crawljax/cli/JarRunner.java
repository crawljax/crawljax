...
public class JarRunner {
	...
	private Options getOptions() {
		...
		options.addOption("b", "browser", true, "browser type: "
				+ availableBrowsers() + ". Default is Firefox");
		...
		options.addOption("o", OVERRIDE, false,
				"Override the output directory if non-empty");
		...
		options.addOption(LOG_FILE, true,
	...
	private String getCrawljaxVersion() {
		try {
			return Resources.toString(
					JarRunner.class.getResource("/project.version"),
					Charsets.UTF_8);
		} catch (IOException e) {
	...
	private CrawljaxConfiguration readConfig(String urlValue, String outputDir) {
		...
		List<Plugin> classPathPlugins = PluginImporter.getPluggedServices(
				Plugin.class, PluginImporterHelper.getDirsFromClassPath());
		...
		if (commandLine.hasOption(CLICK)) {
			builder.crawlRules().click(
					commandLine.getOptionValue(CLICK).split(","));
		} else {
	...
	private void configureTimers(CrawljaxConfigurationBuilder builder) {
		...
		if (commandLine.hasOption(WAIT_AFTER_EVENT)) {
			long time = Long.parseLong(commandLine
					.getOptionValue(WAIT_AFTER_EVENT));
			builder.crawlRules().waitAfterEvent(time, TimeUnit.MILLISECONDS);
		}
		if (commandLine.hasOption(WAIT_AFTER_RELOAD)) {
			long time = Long.parseLong(commandLine
					.getOptionValue(WAIT_AFTER_RELOAD));
			builder.crawlRules()
					.waitAfterReloadUrl(time, TimeUnit.MILLISECONDS);
		}
	}
