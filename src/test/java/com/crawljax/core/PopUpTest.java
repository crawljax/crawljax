...
public class PopUpTest {
	...
	public void run() {
		try {
			crawljax.run();
			assertEquals("Clickables", 2, crawljax.getSession().getStateMachine()
			        .getStateFlowGraph().getAllEdges().size());

			assertEquals("States", 3, crawljax.getSession().getStateMachine().getStateFlowGraph()
			        .getAllStates().size());
