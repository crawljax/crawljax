...
import static com.crawljax.core.configuration.CrawljaxConfiguration.CrawljaxConfigurationBuilder;
...
import com.crawljax.plugins.ClickableDetectorFilter;
...
public class ClickableDetectorTest {
	...
	public void clickEverything() throws MalformedURLException {
		...
		System.out.println("\n\n clickEverything States: " + session.getStateFlowGraph().getAllStates().size()
		  + " Edges: " + session.getStateFlowGraph().getAllEdges().size());
	...
	private CrawljaxConfigurationBuilder builderForClickDetector() {
		...
		conf.setProxyConfig(proxyPlugin.getConfiguration())
	...
	private CrawljaxConfigurationBuilder crawlDemoSetup() {
		return CrawljaxConfiguration.builderFor("http://demo.crawljax.com")
									.addPlugin(interceptor)
									.setBrowserConfig(
									  new BrowserConfiguration(EmbeddedBrowser.BrowserType.CHROME));
	}

	@Test
	public void clickableDetectorShouldFindSameStatesAsWithout() throws MalformedURLException, ExecutionException, InterruptedException {
		ExecutorService executorService = Executors.newFixedThreadPool(2);
		Future<CrawlSession> withoutClickDetect = executorService.submit(new CrawljaxRunner(crawlDemoSetup().build()));
