...
/**
 * A filter that inserts JavaScript into every HTML file that intercepts clickables. When an element is clickable, it
 * receives the <code>data-cj-clickable</code> attribute which can be picked up by Crawljax.
 *
 * <p>
...
public class ClickableDetectorFilter extends JavaScriptInjectorFilterSource {
	...
	private static final String MUTATION_LIBRARY =
	  "<SCRIPT src=\"https://mutation-summary.googlecode.com/git/src/mutation-summary.js\"></SCRIPT>";

	public static final String[] TAGS = { "A", "ADDRESS", "AREA",
	  "ARTICLE", "ASIDE", "BIG", "BLOCKQUOTE", "BODY", "BUTTON", "CANVAS", "CITE", "CODE",
	  "COL", "COLGROUP", "DD", "DL", "DT", "DIV", "FIELDSET", "FIGCAPTION", "FIGURE",
	  "H1", "H2", "H3", "H4", "H5", "H6", "HEADER", "IMG", "INPUT", "LABEL", "LEGEND", "LI",
	  "NAV", "OBJECT", "P", "PRE", "SECTION", "SELECT", "SMALL", "TABLE", "TBODY", "TD", "TH", "TITLE",
	  "TR", "TT", "UL" };
	...
	private static String wrapAs(String tag, String content) {
		return System.lineSeparator() + '<' + tag + ">" + System.lineSeparator() + content + System.lineSeparator()
		  + "</" + tag + '>';
	}

	public static ClickableDetectorFilter withCss() {
		String css = wrapAs("SCRIPT", readFile(JS_FILE));
		String javaScript = wrapAs("STYLE", readFile(CSS_FILE));
	 ...
	 * <p>
	 * Unfortunately crawljax does not (yet) support setting a wildcard for the elements. Until then, this
	 * method helps you setup the clickable detector for all (sane) elements where a click action
	 * might be registered to.
	 * </p>
	 *
	 * @param rules The rule builder whe the click rules will be set.
	 */
