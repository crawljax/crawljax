...
/**
 * Starts a proxy server along with Crawljax. The proxy server can be used to monitor traffic and to modify
 * incoming resources.
 */
public class CrawljaxProxyPlugin implements PreCrawlingPlugin, PostCrawlingPlugin {
	...
	/**
	 * @param port The port you want the proxy to run at.
	 */
	...
	public CrawljaxProxyPlugin addFilter(HttpFiltersSource filter) {
		Preconditions.checkState(server == null, "You cannot modify the list once the server has started");
		filters.add(filter);
		return this;
	}

	/**
	 * @return The proxy configuration that should be set in {@link com.crawljax.core.configuration
	 * .CrawljaxConfiguration.CrawljaxConfigurationBuilder#setProxyConfig(com.crawljax.core.configuration
	 * .ProxyConfiguration)}
	 */
