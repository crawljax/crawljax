...
class OutputBuilder {
    ...
    /**
     * @param outputDir target for the output directory. Folder must not exist or be empty.
     */
    ...
    private void copySkeleton() {
        ...
        if (skeleton.toExternalForm().contains("jar!")) {
            ...
            try {
                ...
                throw new CrawljaxException("Could not copy required resources: " + e.getMessage(), e);
            }
        }
    }
    ...
    private void copySkeletonFromJar(URL skeleton) {
        ...
        try (ZipInputStream zis = new ZipInputStream(new FileInputStream(jar))) {
            ...
            throw new CrawljaxException("Could not copy required resources: " + e1.getMessage(), e1);
        }
    }
    ...
    private File getJar(URL skeleton) {
        ...
        try {
            ...
            throw new CrawljaxException("Could not process the path of the Overview skeleton " + skeleton, e);
        }
    ...
    private AlchemyGraphModel getAlchemyJson(OutPutModel model) {
        ...
        for (Edge edge : model.getEdges()) {
            ...
            edges[i] =
                    new AlchemyEdge(from, to, edge.hashCode(), edge.getText(), edge.getElement(), edge.getEventType());
            i++;
        }
