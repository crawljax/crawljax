...
public class TestSuiteGenerator implements PostCrawlingPlugin {
  ...
  public void postCrawling(CrawlSession session, ExitStatus exitReason) {
    ...
    checkCrawlPaths(session.getCrawlPaths(), session.getStateFlowGraph().getAllEdges(),
  ...
  private void checkCrawlPaths(Collection<List<Eventable>> collection,
      ImmutableSet<Eventable> immutableSet, List<Eventable> expiredEdges) {
    ...
    for (List<Eventable> path : collection) {
      ...
      for (Eventable event : path) {
        if (!map.containsKey(event.getId())) {
          LOGGER.error("Found an eventable not in the graph {} from {} to {}", event.getId(),
              event.getSourceStateVertex().getName(), event.getTargetStateVertex().getName());
...
//			                this.testgenHostInterface.getParameters().get("testRecordsDir"),
      ...
      String generatedFileName = generator.generate(
          DomUtils.addFolderSlashIfNeeded(absPath + TEST_SUITE_PATH),
          FILE_NAME_TEMPLATE);
      if (null != generatedFileName) {
        generator.copyExecutionScripts(absPath, TEST_SUITE_SRC_FOLDER, TEST_SUITE_PACKAGE_NAME,
            CLASS_NAME);
      }
