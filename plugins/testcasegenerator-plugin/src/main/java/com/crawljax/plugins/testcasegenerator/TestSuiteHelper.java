...
public class TestSuiteHelper {
   ...
   */
  public TestSuiteHelper(CrawljaxConfiguration config, String jsonStates, String jsonEventables,
      String crawlPath,
  ...
  public void init(CrawljaxConfiguration config, String jsonStates, String jsonEventables,
      String crawlPath,
  ...
  private void loadCrawljaxCore(CrawljaxConfiguration config, String url) {
    ...
    this.invariantChecker = new ConditionTypeChecker<Invariant>(
  ...
  private void loadHybridState(StateVertex state, FragmentManager fragmentManager)
      throws IOException {
  ...
  private void cleanEventables() {
    for (Eventable eventable : mapEventables.values()) {
      ...
      Element element = new Element(eventable.getElement().getTag(),
          eventable.getElement().getText(),
          attributes);
      eventable.setElement(element);
    }
  }

  TestRecord newTestRecord(String methodName, List<TestRecord> testRecords,
...
//				WorkDirManager.TEST_RESULTS_HTML_VM;
    ...
    manager.saveTestRecordMap(testRecords, manager.getNumTestRecords(new File(testRecordsFolder)),
        url,
  ...
  public boolean loadOldPage(String domString, String url) {
    ...
    try {
      ((JavascriptExecutor) driver)
          .executeScript("document.getElementsByTagName('html')[0].innerHTML=arguments[0]",
              domString);
    } catch (Exception ex) {
  ...
  public StateComparision fragDiff(StateVertex oldState, StateVertex newState,
    ...
    StateComparision comp = fragmentManager.cacheStateComparision(newState, oldState,
        assignDynamic);
  ...
  private void saveScreenshot(EmbeddedBrowser browser, String name, File newScreenshot) {
    ...
    try {
      ...
      LOGGER.warn("Screenshots are not supported or not functioning for {}. Exception message: {}",
          browser,
          e.getMessage());
  ...
  public boolean fragDiffBool(long stateVertexId) {
    ...
    boolean comp = setStateComparison(vertex, newVertex, fragmentManager, reportBuilder,
        currTestRecord, false);

    return comp;
  }

  private boolean setStateComparison(StateVertex vertex, StateVertex newVertex,
      FragmentManager fragmentManager,
    ...
    StateComparision comparision = fragDiff(vertex, newVertex, fragmentManager, reportBuilder,
        currTestRecord,
        mutant);
  ...
  public StateComparision fragDiff(StateVertex vertex, StateVertex newVertex,
      FragmentManager fragmentManager,
      ReportBuilder reportBuilder, TestRecord currTestRecord, boolean mutant) {
    try {
      return fragDiffExcept(vertex, fragmentManager, reportBuilder, currTestRecord, mutant,
          newVertex);
    } catch (Exception e) {
      // TODO Auto-generated catch block
      e.printStackTrace();
  ...
  public StateComparision fragDiffExcept(StateVertex vertex, FragmentManager fragmentManager,
    ...
    switch (comp) {
      case DIFFERENT:
        LOGGER.info("State {} is different from cached state {}", newVertex.getId(),
            vertex.getId());
        reportBuilder.markLastStateDifferent();
        break;
      case DUPLICATE:
  ...
  private void exportAnnotations(ImageAnnotations annotation, File output)
