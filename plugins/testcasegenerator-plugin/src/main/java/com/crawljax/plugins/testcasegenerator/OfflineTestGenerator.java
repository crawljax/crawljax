...
public class OfflineTestGenerator {
    ...
    public static Collection<List<Eventable>> getCrawlPathObjects(
            Collection<List<Eventable>> crawlPaths, String absPath) {
        File crawlPathsInfoJson = new File(absPath, "crawlPathsInfo.json");
        Collection<List<Eventable>> returnPaths = new ArrayList<>();
        Gson gson = new Gson();
        Type typeOfObjectsList = new TypeToken<List<CrawlPathInfo>>() {}.getType();
    ...
    public static String generateTestCases(
            Collection<List<Eventable>> crawlPaths, OutPutModel outputModel, JsonObject configObject, String absPath) {
        ...
        if (crawlPathsJson.exists()) {
            ...
            try {
                Collection<List<Eventable>> crawlPaths = gson.fromJson(
                        ...
                        new TypeToken<ArrayList<CrawlPath>>() {}.getType());
    ...
    private static boolean replaceJsonEventables(Map<Long, Eventable> mapEventables, File jsonEventables) {
        ...
        try {
            TestSuiteGeneratorHelper.writeEventableTestDataToJson(
                    jsonEventables.getAbsolutePath(), mapEventables.values());
        } catch (IOException e) {
    ...
    private static List<Eventable> getEventable(int source, int target, List<Eventable> allEventables) {
        ...
        for (Eventable eventable : allEventables) {
            ...
            int thisSource = (int) Double.parseDouble(
                    ((LinkedTreeMap<?, ?>) eventable.getEdgeSource()).get("id").toString());
            int thisTarget = (int) Double.parseDouble(
                    ((LinkedTreeMap<?, ?>) eventable.getEdgeTarget()).get("id").toString());
    ...
    private static Eventable getEventableFromGraph(
            String source, String target, Map<Long, Eventable> mapEventables, OutPutModel model) {
        ...
        if (found != null) {
            ...
            for (Eventable eventable : mapEventables.values()) {
                ...
                if (found.getIdentification()
                        .getValue()
                        .equalsIgnoreCase(eventable.getIdentification().getValue())) {
    ...
    private static int getStateId(Object state) {
        if (state instanceof StateVertex) {
            ...
            return (int)
                    Double.parseDouble(((LinkedTreeMap<?, ?>) state).get("id").toString());
        }
    }
    ...
    private static List<Eventable> fixCrawlPath(
            List<Eventable> crawlPath,
        ...
        for (Eventable eventable : crawlPath) {
            ...
            if (eventable.getEdgeSource() instanceof StateVertex) {
                ...
                source = (int) Double.parseDouble(((LinkedTreeMap<?, ?>) eventable.getEdgeSource())
                        .get("id")
                        .toString());
            }
            ...
            if (target != -1 && source != target) {
                ...
                if (fix != null) {
                    ...
                    LOGGER.info(
                            "Found a fix for the path with eventable {}",
                            fix.get(0).getId());
                } else {
                    LOGGER.error(
                    ...
                    if (sourceState instanceof StateVertex) {
                        ...
                        src = ((LinkedTreeMap<?, ?>) eventable.getEdgeSource())
                                .get("name")
                                .toString();
                    }
            ...
            if (eventable.getEdgeTarget() instanceof StateVertex) {
                ...
                target = (int) Double.parseDouble(((LinkedTreeMap<?, ?>) eventable.getEdgeTarget())
                        .get("id")
                        .toString());
            }
    ...
    private static List<List<Eventable>> fixCrawlPaths(
            Collection<List<Eventable>> crawlPaths, Map<Long, Eventable> mapEventables, OutPutModel model) {
