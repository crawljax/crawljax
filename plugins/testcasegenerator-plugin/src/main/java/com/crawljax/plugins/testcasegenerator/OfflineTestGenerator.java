...
public class OfflineTestGenerator {
  ...
  public static final String JSON_EVENTABLES =
      TestSuiteGenerator.TEST_SUITE_PATH + "eventables.json";
  private static final Logger LOGGER = LoggerFactory.getLogger(
      OfflineTestGenerator.class.getName());
...
//				source = event.getSourceStateVertex().getName();
  ...
  public static Collection<List<Eventable>> getCrawlPathObjects(
      Collection<List<Eventable>> crawlPaths,
      String absPath) {
    ...
    try {
      ...
      for (List<Eventable> crawlPath : crawlPaths) {
        if (!verifyCrawlPathInfo(crawlPath, pathsInfo.get(index))) {
          System.err.println(
              "Error building CrawlPath Object for " + crawlPath + ":" + pathsInfo.get(index));
          return null;
        }
  ...
  public static String generateTestCases(Collection<List<Eventable>> crawlPaths,
    ...
    String browserType = configObject.get("browserConfig").getAsJsonObject().get("browserType")
        .getAsString();
    ...
    int WAIT_TIME_AFTER_RELOAD = configObject.get("crawlRules").getAsJsonObject()
        .get("waitAfterReloadUrl")
        .getAsInt();
    int WAIT_TIME_AFTER_EVENT = configObject.get("crawlRules").getAsJsonObject()
        .get("waitAfterEvent").getAsInt();
    builder.crawlRules().waitAfterReloadUrl(WAIT_TIME_AFTER_RELOAD, TimeUnit.MILLISECONDS);
    builder.crawlRules().waitAfterEvent(WAIT_TIME_AFTER_EVENT, TimeUnit.MILLISECONDS);

    try {
      JavaTestGenerator generator = new JavaTestGenerator(TestSuiteGenerator.CLASS_NAME, initialURL,
          testMethods,
          config, // Config,
          absPath,
  ...
  private static Eventable getEventableFromGraph(String source, String target,
      Map<Long, Eventable> mapEventables,
      OutPutModel model) {
    ...
    if (found != null) {
      ...
      for (Eventable eventable : mapEventables.values()) {
        ...
        if (found.getIdentification().getValue()
  ...
  private static List<Eventable> fixCrawlPath(List<Eventable> crawlPath,
      List<Eventable> allEventables,
