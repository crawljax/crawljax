...
//		updateIdsForEventables();
  ...
  public static void writeEventableTestDataToJson(String fname, Collection<Eventable> eventables)
      throws IOException {
  ...
  public static String getEventableInfo(Eventable eventable) {
    if ((eventable.getEdgeSource() instanceof LinkedTreeMap)
      ...
      return MoreObjects.toStringHelper(eventable)
          ...
          .add("source", "" + (int) Double.parseDouble(
              ((LinkedTreeMap<?, ?>) eventable.getEdgeSource()).get("id").toString()))
          .add("target", "" + (int) Double.parseDouble(
...
//			if(pathInfoMap!=null && pathInfoMap.containsKey(crawlPath)) {
      if (crawlPath instanceof CrawlPath) {
        ...
        CrawlPathInfo info = new CrawlPathInfo(iter, pathCast.getBacktrackTarget(),
            pathCast.isBacktrackSuccess(), pathCast.isReachedNearDup());
      ...
      for (Eventable clickable : crawlPath) {
        ...
        try {
          ...
          if (clickable.getEdgeSource() instanceof LinkedTreeMap) {
            sourceid = "" + (int) Double.parseDouble(
                ((LinkedTreeMap<?, ?>) clickable.getEdgeSource()).get("id").toString());
          }
        ...
        try {
          ...
          if (clickable.getEdgeTarget() instanceof LinkedTreeMap) {
            targetid = "" + (int) Double.parseDouble(
                ((LinkedTreeMap<?, ?>) clickable.getEdgeTarget()).get("id").toString());
          }
