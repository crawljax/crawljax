...
public final class VipsOutput {
  ...
  private void writeVisualBlocks(Element parentNode, VisualStructure visualStructure, int level) {
    ...
    layoutNode.setAttribute("FrameSourceIndex",
        String.valueOf(visualStructure.getFrameSourceIndex()));
    layoutNode.setAttribute("SourceIndex",
...
//					layoutNode.setAttribute("Content", content);
  ...
  public int drawVisualStructure(VisualStructure visualStructure, Graphics2D g2d,
      List<VipsRectangle> vipsRectangles, int id, int parentId, WebDriver driver) {
...
//		LOG.info("DOM:" + rect1);
    ...
    if (!rect2.contains(rect1) && !rect1.contains(rect2) && rect1.width * rect1.height > 0
...
//		LOG.info("final" + finalRect);
    VipsRectangle me = new VipsRectangle(visualStructure.getNestedBlocks(), id, parentId,
        XPathHelper.getXPathExpression(vipsBlock), finalRect);
  ...
  public List<VipsRectangle> exportVisualStructureToImage(VisualStructure visualStructure,
      BufferedImage pageViewport, File target, boolean fragOutput, WebDriver driver) {
    LOG.debug(visualStructure.getNestedBlocks().toString());
    BufferedImage image = new BufferedImage(pageViewport.getWidth(), pageViewport.getHeight(),
        pageViewport.getType());
